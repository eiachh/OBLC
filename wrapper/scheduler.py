...
class Scheduler:
    ...
    def startScheduler(self):
        if((not hasattr(self, 'scheduledQueueConsumer')) or self.scheduledQueueConsumer.is_running == False):
            self.scheduledQueueConsumer = RepeatedTimer(Scheduler.getScheduledQueueRecheckInterval, self.iterateScheduledQueue)
        if((not hasattr(self, 'priorityQueueConsumer')) or self.priorityQueueConsumer.is_running == False):
